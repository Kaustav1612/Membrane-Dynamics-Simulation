...
simulated_area = 15e-12;        % 3.9 μm x 3.9 μm membrane (15 μm²)
membrane_area = 15e-12;            % Membrane area should match simulated area
viscosity = 300;                 % Water viscosity (Pa·s)
...
total_simulation_time = 100;     %100 s total time
surface_tension = 50e-6;  

tau_flicker = 1;        % Persistence time of activity
tau_myo = 20;
flicker_activity = 5;
...
pinning_stiffness = 1e-3;
pin_spacing  = 13e-9;
k_on_pinning = 0.5;
k_off_pinning = 0.2;
max_pin_density = 10;
rng('shuffle');
pin_density_new = randi([1, max_pin_density], simulation_grid_size, simulation_grid_size);
...
tau_cs = 600;       % cytoskeleton remodelling time  in s
s_activity = 0.65;     % activity factor (0..1). Larger s => more assembly relative to detachment
P_max = 0; 
...
for step = 2:num_time_steps
    ...
    % 2) current pinning density
    current_pin_density = pin_density_new;
    pin_density_new = current_pin_density + time_step * (k_on_pinning*(max_pin_density*ones(simulation_grid_size,simulation_grid_size)...
        -current_pin_density)- k_off_pinning * exp(pinning_stiffness.*height_current * 1e-9/ (boltzmann_const * temperature)));
    ...
    pinning_map = zeros(simulation_grid_size, simulation_grid_size);
    pinning_map(find(cs_presence>0)) = 1;
...
% --- Define Spatial Correlation Lengths ---
    % These represent the physical size of correlated active patches (adjust units as needed)
    lambda_flicker = 50e-9;  % Example: 50 nm correlation length for channel activity
    lambda_myo     = 200e-9; % Example: 200 nm correlation length for myosin stress
    ...
    % --- 2. EVOLVE CHANNEL FLICKER (Method 2) ---
    eta_flicker = makeHermitian(randn(size(Hq)) + 1i*randn(size(Hq)));
    
    % Method 2: The relaxation rate becomes q-dependent
    % Effective tau(q) = tau_flicker / (1 + lambda^2 * q^2)
    decay_rate_flicker = (1 + lambda_flicker^2 * Q2) ./ tau_flicker;
    
    % Euler-Maruyama Update with q-dependent decay
    d_Xi_flicker = -(Xi_flicker_q .* decay_rate_flicker) * time_step + ...
                    amp_flicker * eta_flicker * sqrt(time_step);
                    
    Xi_flicker_q = Xi_flicker_q + d_Xi_flicker; 
    
    
    % --- 3. EVOLVE MYOSIN STRESS (Method 2) ---
    eta_myo = makeHermitian(randn(size(Hq)) + 1i*randn(size(Hq)));
    
    % Method 2: q-dependent decay for the myosin stress field
    decay_rate_myo = (1 + lambda_myo^2 * Q2) ./ tau_myo;
    
    % Euler-Maruyama Update with q-dependent decay
    d_Xi_myo_stress = -(Xi_myo_stress_q .* decay_rate_myo) * time_step + ...
                       amp_myo * eta_myo * sqrt(time_step);
                       
    Xi_myo_stress_q = Xi_myo_stress_q + d_Xi_myo_stress; 
    
    
    % --- 4. CALCULATE FINAL FORCES ---
    
    % Force A: Channel Flicker (Direct force application)
    Force_Flicker = Xi_flicker_q;
    
    % Force B: Myosin (Coupled to Curvature)
    % This implementation REMAINS THE SAME. It correctly applies your now spatially-correlated 
    % stress field to the membrane curvature.
    Force_Myosin  = Xi_myo_stress_q .* (-Q2 .* Hq);
    
    % Total Active Force
    Total_Active_Force = Force_Flicker + Force_Myosin; % Added Force_Myosin here as it was missing in your snippet
...
%     Sigma_area = (K_A / (2 * membrane_area)) * I_grad;
    ...
    F_pin = -pinning_map .* pinning_stiffness * pin_density_new  * height_current;
    F_pin_passive = -pinning_map .* pinning_stiffness * pin_density_new * height_current_passive;
    ...
    F_full_real = F_bend_full + F_tens_full+ F_pin;
    F_full_real_passive = F_bend_full_passive + F_tens_full_passive;
...
eq_pin_density = (k_on_pinning*max_pin_density*ones(simulation_grid_size,simulation_grid_size))/(k_off_pinning * exp(pinning_stiffness.*height_current*1e-9/ (boltzmann_const * temperature)) + k_on_pinning);
confinement = (pinning_stiffness * eq_pin_density);
mean_confinement = mean(confinement(:));
...
for i = 1:length(freq_Hz_global)
    ...
    flicker_Da_q = (flicker_activity);  
    contract_Da_q = contract_activity * (q.^2);
...
%%
T_eff = 310*(PSD_theoretical./PSD_theoretical_passive);
%%
...
avg_std_val=[];
for k = 1 : 10000 :size(height_time_series,3)
    ht = height_time_series(7,7,(k:k+9999));
    std_dev_time = std(ht,0,3);
%     figure;
...
%     title('Standard Deviation of height(nm)');
    avg_std_val = [avg_std_val,mean(std_dev_time(:))];
end
figure();
plot(avg_std_val);

figure;
ht = height_time_series;
std_dev_time = std(ht,0,3);
imshow(std_dev_time*1e9,[min(std_dev_time(:)*1e9) max(std_dev_time(:)*1e9)]);
colormap jet;
colorbar;
