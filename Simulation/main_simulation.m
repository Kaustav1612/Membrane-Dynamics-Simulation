...
tau_myo = 0;
flicker_activity = 2.5*temperature;
contract_activity = 0;
...
all_PSD = NaN(simulation_grid_size - 1, num_freq_points);
...
for k = 1:simulation_grid_size - 1
    ...
    h_patch = height_time_series(k, k, :) * 1e9; 
    ...
    if mod(k, 10) == 0, fprintf('Processed %d/%d\n', k, simulation_grid_size-1); end
end

% 5. Ensemble Average across all spatial points
averaged_PSD = mean(all_PSD, 1, 'omitnan');
freq_Hz_global = freq_Hz_global'; % Ensure row vector
...
for i = 1:length(freq_Hz_global)
    ...
    integrand_3  = 1 ./ ((omega)^2 + denom_mech_1) .* S_F_q_omega; 
    ...
    PSD_theoretical_passive(i) = ((4 * eta_eff)*kB*T_kelvin/pi) * trapz(q, integrand_1);
...
figure('Color','w'); 
loglog(freq_Hz_global, averaged_PSD, 'b--', 'LineWidth', 1); hold on;
...
for d = 1:num_domains
    ...
    mask = (freq_Hz_global >= f_start & freq_Hz_global <= f_end & ...
            averaged_PSD > 0 & ...
            PSD_theoretical_irm > 0 & PSD_theoretical_passive > 0);
    
    if any(mask)
%         r2_results(d, 1) = calc_R2(averaged_PSD(mask), PSD_theoretical(mask));
        r2_results(d, 2) = calc_R2(averaged_PSD(mask), PSD_theoretical_irm(mask));
        r2_results(d, 3) = calc_R2(averaged_PSD(mask), PSD_theoretical_passive(mask));
    else
        r2_results(d, :) = NaN;
    end
...
fitparam_file = fullfile(dir, 'Fitting parameters.txt'); % Renamed 'dir' to 'data_dir' (see note below)
...
for d = 1:num_domains
    ...
    fprintf(fidx, 'R-square value for %f Hz to %f Hz for theoretical fitting: %.3e\n', ...
...
T = table(DomainNames, r2_results(:,2), r2_results(:,3), ...
    'VariableNames', {'Frequency_Range', 'R2_Theory', 'R2_Passive'});
disp(T);

% --- 5. Final Visualization ---
figure('Color','w','Position',[100 100 900 600]);
h1 = loglog(freq_Hz_global, averaged_PSD, 'g--', 'LineWidth', 1.2); hold on;
% h2 = loglog(freq_Hz_global, PSD_theoretical, 'r-', 'LineWidth', 2);
h3 = loglog(freq_Hz_global, PSD_theoretical_irm, 'b-', 'LineWidth', 2);
h4 = loglog(freq_Hz_global, PSD_theoretical_passive, 'm-', 'LineWidth', 2);

% Shading and Text Labels for Domains
colors = lines(num_domains);
y_lims = ylim;
for d = 1:num_domains
    f_s = domain_ranges(d, 1); f_e = domain_ranges(d, 2);
    fill([f_s f_e f_e f_s], [y_lims(1) y_lims(1) y_lims(2) y_lims(2)], ...
         colors(d,:), 'FaceAlpha', 0.05, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    text(sqrt(f_s*f_e), y_lims(2)*0.5, sprintf('D%d', d), 'HorizontalAlignment', 'center', 'FontWeight', 'bold');
end

% Build a custom legend that references the table or specific domain
legend_str = {
    'Simulated Data', ...
    ['Model'], ...
    ['Passive (Thermal) Model']
};
legend([h1 h3 h4], legend_str, 'Location', 'southwest');
xlabel('Frequency (Hz)'); ylabel('PSD (nm^2/Hz)');
...
% --- 1. Compute the ACF ---
% Using the fast_acf logic from before
h_signal = h_series;
N = length(h_signal);
...
[fit_result, gof] = fit(lags(1:fit_limit), acf(1:fit_limit), f_type, options);
tau_fit = fit_result.tau;

% --- 4. Plotting ---
figure('Color', 'w');
plot(lags, acf, 'LineWidth', 3, 'Color', [0.5 0.5 0.5], 'DisplayName', 'Raw ACF');
hold on;
plot(fit_result, '--b');
...
xlim([0, tau_fit*1.5 ]); % Zoom in on the relevant decay
grid on;
