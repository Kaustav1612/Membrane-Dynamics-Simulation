...
total_simulation_time = 50e-3;     % 10 ms total timefTpo
surface_tension = 0.1e-3;           % MUCH lower tension (0.1 pN/um)
mu = 1e-6;                       % Shear Modulus (N/m)                  
old_dir = pwd;
output_dir_irm = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\03\results_irm';            
output_dir_moransI = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\03\results_moransI';
output_dir_protein_map = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\03\results_protein_map';
output_dir_curvature_map = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\03\results_curvature_map';
pinning_strength = 5e-6;          % Moderate pinning (N/m²)
...
low_tension = 30e-6;
high_tension  = 100e-6;
curvature_radius = 2e14;
C_p = 1/curvature_radius; % 1/m, spontaneous curvature per unit protein coverage
...
% Initialize arrays
height_current = 30e-9 * randn(simulation_grid_size);
...
A_hat = 150;     % interaction parameter
S_hat = 100;     % saturation denstiy parameter
phi0_mean = 0.1;    % mean area fraction
sigma_init = 0.01;  % noise strength
...
for step = 2:num_time_steps
    ...
    if step > 20
        term4 = -kappa_map.*(Laplacian_h-C_p*phi);
    ...
    % === Time update (Euler for now) ===
    phi_new = phi + time_step * mu;
    ...
    if mod(step, 100) == 1
        update_visualization(fig1, X_nm, Y_nm,L_nm,height_current, time_points(step));
        drawnow;
    end
end
%%
cd('D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\03');
% Setup for video
...
for t = 1:num_time_steps
    ...
    surf(X_nm/L_nm, Y_nm/L_nm, h, 'EdgeColor','none');
    hold on;
...
fig2 = figure('Position', [100, 100, 600, 600], 'Color', 'w');
fig3 = figure('Position', [100, 100, 600, 600], 'Color', 'w');
fig4 = figure('Position', [100, 100, 600, 600], 'Color', 'w');
fig5 = figure('Position', [100, 100, 600, 600], 'Color', 'w');
for step = 1:num_time_steps
    if rem(step,10) == 1
        ...
        colorbar;
        axis equal tight;
        title(sprintf('Intensity at Step %d', step));
        drawnow;

        % Display Moran's I
        figure(fig3);
        imagesc(morans_i_map);
        colormap(turbo);
        colorbar;
        axis equal tight;
        title(sprintf('Moran''s I Map at Step %d', step));
        drawnow;

        
        figure(fig5);
        imagesc(protein_map_series(:,:,step));
        colormap(turbo);
        colorbar;
        axis square; colorbar;
        title('Protein density Map in kBT');
        xlabel('X (pixels)'); ylabel('Y (pixels)');
...
function [S2D, qx, qy, q_r, S_radial, S_qw, omega, q_list] = compute_PSDs(h_ts, dx, dy, dt)
    ...
    Nx = size(h_ts,2); Ny = size(h_ts,1); % if orientation differs, ensure consistent: here assume (Ny x Nx x Nt)
    [Ny, Nx, Nt] = size(h_ts);
...
function update_visualization(fig, X_nm, Y_nm, L_nm, h_total, current_time)
    figure(fig);
    clf;
    ...
    % Cross-section
    subplot(2,2,4);
    mid = round(size(h_nm,1)/2);
    plot(X_nm(mid,:), h_nm(mid,:), 'b-', 'LineWidth', 1.5);
    title(sprintf('Cross-section at y = %.0f nm', Y_nm(mid,1)));
    xlabel('x (nm)'); ylabel('Height (nm)');
    grid on;
    drawnow;
end
