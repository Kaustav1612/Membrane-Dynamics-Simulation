...
surface_tension = 500e-6;           % MUCH lower tension (0.1 pN/um)
mu = 1e-6;                       % Shear Modulus (N/m)                  
old_dir = pwd;
dir = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\raft_loving_protein';
output_dir_irm = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\raft_loving_protein\results_irm';            
output_dir_moransI = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\raft_loving_protein\results_moransI';
output_dir_bending_map = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\raft_loving_protein\bending_map';
output_dir_lipid_map = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\raft_loving_protein\results_lipid_map';
output_dir_protein_map = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\raft_loving_protein\results_protein_map';
output_dir_curvature_map = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\raft_loving_protein\results_curvature_map';
pinning_strength = 50e-6;          % Moderate pinning (N/m²)
...
J = 0.85;
lambda = 0.01;
psi0_mean = 0.8;    % mean area fraction
...
tau = 1000* time_step;            % relaxation time
tau_ramp = 1000*time_step;       % how fast remodeling ramps up (smoothness)
t0 = 2000 * time_step;      % start remodeling at step 100
...
kappa_variation = 0.01 * base_kappa;
gamma_variation = 0.01 * base_gamma;
...
for step = 2:num_time_steps
    ...
    %Positive J refers to proteins to nucleate psi =1 rafts
    mu_psi = alpha .* (psi.^3 - psi) - lambda .* Laplacian_psi - J*(psi-phi) ;
    

    % Protein area fraction (Cahn–Hilliard(from Rangamani Paper)+ coupling + height term + actin attachment)
    s = 1 - exp( -(step*time_step - t0) / tau_ramp );   % 0..1 smooth turn-on, set s=0 for t<t0 if desired
    s = max(0, s);   % ensure non-negative
    mu_phi = term1 + term2 + term3  ...
        - kappa_map.*(Laplacian_h - C_p.*phi) ...
        - (s ./ tau) .* (phi - phi_eq); % Activity is involved
    ...
    % Enforce bounds (since phi is area fraction)
    phi_new = min(max(phi_new, 0), 1);
    % Store
    protein_map_series(:,:,step) = phi_new;
    ...
    if mod(step, 5000) == 1
        update_visualization(fig1,fig2, X_nm, Y_nm,L_nm,height_current, time_points(step),lipid_map_series,kappa_map_series,protein_map_series,step);
        drawnow;
    end
end
%%
...
function update_visualization(fig1,fig2,X_nm, Y_nm, L_nm, h_total, current_time,lipid_map_series,kappa_map_series,protein_map_series,step)
    figure(fig1);
    clf;
