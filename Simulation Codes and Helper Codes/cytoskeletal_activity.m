...
simulated_area = 5e-12;        % 5μm x 5μm membrane (5 μm²)
membrane_area = 5e-12;            % Membrane area should match simulated area
viscosity = 10;                 % effective viscosity (Pa·s)
...
time_step = 20e-6;                 % MUCH smaller time step (20 ms)
total_simulation_time = 5;     % 5 s total time
surface_tension = 40e-6;           % high tension (60 pN/um)
old_dir = pwd;
% shear_moduli = ;
dir = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\passive\low_curvature';
% output_dir_irm = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\Cyto_love_raft_love_protein\activity_2.5\results_irm';            
% output_dir_moransI = 'D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active\Cyto_love_raft_love_protein\activity_2.5\results_moransI';
...
C_p = 0.05e9;                % protein curvature [1/m]
C_ld = 0.008e9;               % lipid curvature [1/m]
C_lo = 0.005e9;
...
phi_eq = 0.2;                  % equilibrium protein order
...
A_hat = 50;     % interaction parameter
S_hat = 300;     % saturation denstiy parameter
alpha = 5;    
...
% --- Protein field phi ---
phi = 0.2 * randn(N, N);  % small random fluctuations around 0
...
tau_cs = 660;       
s_activity = 0.2;     % activity factor (0..1). Larger s => more assembly relative to detachment
...
% -----------------------------------------------------------------------
I_grad_all = [];
% cs_presence = double(rand(simulation_grid_size) < cs_init_prob);  % if you have initial prob
% if not defined, create a sparse random mesh to start:
if ~exist('cs_presence','var')
    cs_init_prob = 0.35; % initial fraction of sites with cytoskeleton
...
%   1 --------------------------- CYTOSKELETON TURNOVER (stochastic) --------------------------------
    ...
    %   1 --------------------------- VESICULATION EVENTS (stochastic) --------------------------------
    ...
    if step >100
        F_area_linear = Sigma_area.*Laplacian_h; 
    else
        F_area_linear=0;
    end
%     Sigma_area = (area_compressibility / (2 * membrane_area)) * I_grad;
    ...
    % 14) store & visualize
    height_current = height_current - median(height_current(:));
    height_time_series(:,:,step) = height_current;
    % Update visualization less frequently
    if mod(step,5000) == 1
        update_visualization(fig1,fig2, X_nm, Y_nm,height_current, time_points(step),lipid_map_series,kappa_map_series,protein_map_series,step,cs_presence);
        drawnow;
    end
end
...
%%
% cd('D:\Kaustav\CODES SET\Membrane_Sheet_Simulation\protein_present\active');
% % Setup for video
...
function update_visualization(fig1,fig2,X_nm, Y_nm, h_total, current_time,lipid_map_series,kappa_map_series,protein_map_series,step,cs_presence)
    figure(fig1);
    clf;
